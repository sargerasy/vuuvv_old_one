...
var settings = {"qx.globalErrorHandling":"on","qx.theme":"vuuvv.theme.Theme"};
...
var libinfo = {"__out__":{"sourceUri":"../script"},"qx":{"resourceUri":"../resource","sourceUri":"../script","version":"1.3"},"testrunner":{"resourceUri":"../resource","sourceUri":"../script","version":"trunk"},"vuuvv":{"resourceUri":"../resource","sourceUri":"../script","version":"trunk"}};
...
this.setTestNamespace(this.__Ci());
...
return;
}},__Ci:function(){var h=window.location.search;
...
this.__BM=R;
this.__BN=R.childNodes[1];
this.__BO=R.childNodes[2].firstChild;
this.__BT();
qx.log.Logger.register(this);
qx.core.ObjectRegistry.register(this);
},dispose:function(){qx.event.Registration.removeListener(document.documentElement,h,this.__BU,this);
qx.log.Logger.unregister(this);
},clear:function(){this.__BN.innerHTML=k;
},process:function(U){this.__BN.appendChild(qx.log.appender.Util.toHtml(U));
this.__BP();
},__BP:function(){this.__BN.scrollTop=this.__BN.scrollHeight;
},__BQ:true,toggle:function(){if(!this.__BM){this.init();
}else if(this.__BM.style.display==i){this.show();
}else{this.__BM.style.display=i;
}},show:function(){if(!this.__BM){this.init();
}else{this.__BM.style.display=z;
this.__BN.scrollTop=this.__BN.scrollHeight;
}},__BR:[],execute:function(){var X=this.__BO.value;
...
V.className=b;
this.__BR.push(X);
this.__BS=this.__BR.length;
this.__BN.appendChild(V);
this.__BP();
...
}},__BT:function(e){this.__BN.style.height=(this.__BM.clientHeight-this.__BM.firstChild.offsetHeight-this.__BM.lastChild.offsetHeight)+"px";
},__BU:function(e){var bb=e.getKeyIdentifier();
if((bb=="F7")||(bb=="D"&&e.isCtrlPressed())){this.toggle();
e.preventDefault();
}if(!this.__BM){return;
}if(!qx.dom.Hierarchy.contains(this.__BM,e.getTarget())){return;
}if(bb=="Enter"&&this.__BO.value!=""){this.execute();
this.__BO.value="";
}if(bb=="Up"||bb=="Down"){this.__BS+=bb=="Up"?-1:1;
this.__BS=Math.min(Math.max(0,this.__BS),this.__BR.length);
var ba=this.__BR[this.__BS];
this.__BO.value=ba||"";
this.__BO.select();
}}},defer:function(bc){qx.event.Registration.addListener(document.documentElement,h,bc.__BU,bc);
}});
})();
(function(){var l="'>",k="[",h=", ",g="</span>",f="<span class='type-",e="</span> ",d="}",c="",b="]",a="\n",E="{",D="map",C="function",B="<span class='object'>",A="]:",z="<span class='object' title='Object instance with hash code: ",y="string",x="level-",w="0",v="<span class='offset'>",s=":",t="qx.log.appender.Util",q="DIV",r="<span>",o="<span class='type-key'>",p="</span>:<span class='type-",m="</span>: ",n=" ",u="]</span>: ";
qx.Class.define(t,{statics:{toHtml:function(F){var P=[];
var M,O,H,J;
P.push(v,this.formatOffset(F.offset,6),e);

if(F.object){var G=F.win.qx.core.ObjectRegistry.fromHashCode(F.object);

if(G){P.push(z+G.$$hash+l,G.classname,k,G.$$hash,u);
}}else if(F.clazz){P.push(B+F.clazz.classname,m);
}var I=F.items;

for(var i=0,N=I.length;i<N;i++){M=I[i];
O=M.text;

if(O instanceof Array){var J=[];

for(var j=0,L=O.length;j<L;j++){H=O[j];

if(typeof H===y){J.push(r+this.escapeHTML(H)+g);
}else if(H.key){J.push(o+H.key+p+H.type+l+this.escapeHTML(H.text)+g);
}else{J.push(f+H.type+l+this.escapeHTML(H.text)+g);
}}P.push(f+M.type+l);

if(M.type===D){P.push(E,J.join(h),d);
}else{P.push(k,J.join(h),b);
}P.push(g);
}else{P.push(f+M.type+l+this.escapeHTML(O)+e);
}}var K=document.createElement(q);
K.innerHTML=P.join(c);
K.className=x+F.level;
return K;
},formatOffset:function(Q,length){var T=Q.toString();
var R=(length||6)-T.length;
var S=c;

for(var i=0;i<R;i++){S+=w;
}return S+T;
},FORMAT_STACK:null,escapeHTML:function(U){return String(U).replace(/[<>&"']/g,this.__BV);
},__BV:function(V){var W={"<":"&lt;",">":"&gt;","&":"&amp;","'":"&#39;",'"':"&quot;"};
return W[V]||"?";
},toText:function(X){return this.toTextArray(X).join(n);
...
if(ba){bh.push(ba.classname+k+ba.$$hash+A);
}}else if(Y.clazz){bh.push(Y.clazz.classname+s);
}var bb=Y.items;
var be,bg;
...
if(be.trace&&be.trace.length>0){if(typeof (this.FORMAT_STACK)==C){bg+=a+this.FORMAT_STACK(be.trace);
}else{bg+=a+be.trace;
}}
...
if(be.type===D){bh.push(E,bc.join(h),d);
}else{bh.push(k,bc.join(h),b);
}}else{bh.push(bg);
}}return bh;
}}});
})();
...
this.__Cj=[];
},members:{__Cj:null,run:function(d,e){var f=c+d.getFullName().replace(/\W/g,b);
this.__Cj.push(f);
window[f]=e;
},exportToJsUnit:function(){var self=this;
window.exposeTestFunctionNames=function(){return self.__Cj;
};
window.isTestPageLoaded=true;
}}});
})();
(function(){var g="vuuvv.test.DemoTest",f="This should never fail!",e="A rose by any other name is still a rose",d="Can false be true?!",c="You must be kidding, 3 can never be outside [1,10]!";
...
},testQuery:function(){var q=vuuvv.Query();
}}});
})();
(function(){var w="",v="completed",u="timeout",t="query",s="failed",r="qx.event.type.Data",q="/admin",o="limit",n="delete",m="=",d="save",l="value",h="vuuvv.Query",c="order",b="conditions",g="&",f="qx.event.type",j="application/json",a="/",k="POST";
qx.Class.define(h,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
},properties:{name:{init:w,nullabel:true},type:{check:[d,n,t],init:t},conditions:{init:[]},order:{init:null,nullable:true},limit:{init:null,nullable:true},value:{init:null,nullable:true}},events:{completed:r,failed:r,timeout:f},members:{query:function(){var x=new qx.io.remote.Request(this.getUrl(),k,j);
x.setData(this.parseData());
x.addListener(v,this._onCompleted,this);
x.addListener(s,this._onFailed,this);
x.addListener(u,this._onTimeout,this);
x.send();
},getUrl:function(){return [q,this.getType(),this.getName()].join(a);
},parseData:function(){var p=[b,c,o,l];
var y=w;

for(var i=0;i<p.length;i++){var name=p[i];
var z=this.get(name);

if(z)y+=encodeURIComponent(name)+m+encodeURIComponent(qx.util.Json.stringify(z))+g;
}return y.substring(0,y.length-1);
},addCondition:function(A,B,C){this.getConditions().push([A,B,C]);
},_onCompleted:function(e){this.fireDataEvent(v,e.getData());
},_onCompleted:function(e){this.fireDataEvent(s,e.getData());
},_onCompleted:function(e){this.fireEvent(u);
}}});
})();
